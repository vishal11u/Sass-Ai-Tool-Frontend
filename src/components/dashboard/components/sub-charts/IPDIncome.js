...
function IPDIncome() {
  ...
  const handleButtonClick = (button) => {
    ...
    fetchLoginData(button);
  };

  const fetchLoginData = async (filter) => {
    setLoading(true);
    try {
      const response = await axios.get(
        `https://best-aitool-backend.vercel.app/dashboard/login-signup/${activeButton}`
      );
      ...
      if (filter === "day") {
        ...
        const monthData = Array.from({ length: 30 }, (_, index) => ({
          name: `Day ${index + 1}`,
          users: index === 0 ? fetchedData.loginCount : 0,
        }));
        setData(monthData);
      }
    } catch (error) {
  ...
  return (
    <div className="w-[39.5%] h-[39vh] border shadow py-1 mt-2 px-2 ml-2 bg-white rounded-md overflow-hidden">
      <div className="flex items-center justify-between space-x-1 border-b pb-1.5 pt-0.5">
        <h1 className="font-semibold text-[14px]">Users Login</h1>
