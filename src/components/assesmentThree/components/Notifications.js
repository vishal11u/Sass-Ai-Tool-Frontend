    ...
    <div className="mr-6">
      <Tooltip title="Notifications" arrow>
        <div className="border rounded-full mt-2 cursor-pointer">
          <Badge
            badgeContent={unreadCount?.unreadCount}
            color="warning"
            onClick={handleOpen}
          >
          ...
          {notifications?.notifications?.length > 0 ? (
            notifications?.notifications?.map((notification) => (
              <div
                key={notification.id}
                className={`bg-${
                  notification?.isRead ? "gray-200" : "blue-100"
                } shadow border px-3 py-1.5 gap-y-1 mb-2 mx-3 rounded-md flex items-center justify-between`}
              >
